$(function () {
	...
	$(".mobile-menu-list .parent").click(function (e) {
		let target = $(e.target);
		if (target.is($(this))) $(this).children().addClass("opened");
	});
	...
	$(".show-left-menu").on("click", function () {
		...
		if ($headerMain.length) {
			...
			$leftMenu
				.find(".left-menu-content-inner")
				.css("width", +$(this).parent(".left-panel").outerWidth());
		}
	});
	...
	$(".open-popup").magnificPopup({
		type: "inline",
		closeMarkup:
			'<button title="%title%" type="button" class="popup-close mfp-close">Закрыть</button>',
	});
